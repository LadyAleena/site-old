...
html( 'heading' => 'Magical weapons', 'code' => sub { story(*DATA, { 'line magic' => $line_magic }) });
...
by ^Gabriel Elias^
This 7 foot long spear is made of white wax wood with an 8 inch long spear head made of a combination alloy—mithryl and admantium at its tip. It has a thick red tassel which is used as a distraction, to catch the enemies blood, and spreads out to a two foot diameter when thrust forward.
...
by Joshua Moore
This is actually a type of magic weapon such as a Defender weapon. It can be any sword or axe. All Magicbane weapons are +1 normally but have the unique quality of granting an additional +2 bonus to hit anyone casting magic or to hit a specific magic item. When a magic item is struck by a Magicbane weapon, its magic is nulled for 1d4 rounds—this includes magic on swords used to parry a Magicbane weapon or armor struck by them. A magic using being in the midst of casting a spell is automatically interrupted and the spell dissolved.
